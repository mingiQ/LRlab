...
class PNA(VisaInstrument):
    ...
    def __init__(self, name="E5071", address='TCPIP::192.168.0.102::INSTR', enabled=True, timeout=0.1):
    ...
    def set_measurement_name(self):
        self.instrument.write('CALC:PAR:SEL CH1_S11_1')
    ...
    def set_display_state(self, state=True):
        ...
        self.instrument.write("DISP:ENAB %d" % enable)
    ...
    def get_display_state(self):
        ...
        :return:
        """
        return self.instrument.query("DISP:ENAB?")

    #### Frequency setup
    def set_start_frequency(self, freq, channel=1):
        self.instrument.write(":SENS%d:FREQ:START %f" % (channel, freq))

    def get_start_frequency(self, channel=1):
        return float(self.instrument.query(":SENS%d:FREQ:START?" % channel))

    def set_stop_frequency(self, freq, channel=1):
        self.instrument.write(":SENS%d:FREQ:STOP %f" % (channel, freq))

    def get_stop_frequency(self, channel=1):
        return float(self.instrument.query(":SENS%d:FREQ:STOP?" % channel))

    def set_center_frequency(self, freq, channel=1):
        self.instrument.write(":SENS%d:FREQ:CENTer %f" % (channel, freq))

    def get_center_frequency(self, channel=1):
        return float(self.instrument.query(":SENS%d:FREQ:CENTer?" % channel))

    def set_span(self, span, channel=1):
        return self.instrument.write(":SENS%d:FREQ:SPAN %f" % (channel, span))

    def get_span(self, channel=1):
        return float(self.instrument.query(":SENS%d:FREQ:SPAN?" % channel))
    ...
    def get_sweep_points(self, channel=1):
        data = self.instrument.query(":sense%d:sweep:points?" % (channel))
        return int(data)
    ...
    def set_sweep_mode(self, mode='CONT'):
        ...
        if mode.lower() not in allowed_modes:
            ...
            self.instrument.write("SENS:SWE:MODE %s" % mode)

    def get_sweep_mode(self):
        """
        Returns the sweep mode.
        """
        data = self.instrument.query("SENS:SWE:MODE?")
        if data is None:
            return None
        else:
            return data.strip()
    ...
    def get_sweep_time(self, channel=1):
        ...
        """
        answer = self.instrument.query('SENS%d:SWE:TIME?'%channel)
    ...
    def set_frequency_offset_mode_state(self, state):
        s = '1' if state else '0'
        self.instrument.write('SENSE:FOM:STATE ' + s)

    def set_port_powers_coupled(self, state=True):
        self.instrument.write("SOUR:POW:COUP %d"%int(state))
    ...
    def setup_two_tone_measurement(self, read_frequency=None, read_power=None, probe_start=None, probe_stop=None, probe_power=None, two_tone=True ):
        if two_tone:
            print("TWO TONE ON")
            self.instrument.write('SENSE:FOM:RANGE4:COUPLED 1')
            self.instrument.write('SENSE:FOM:RANGE2:COUPLED 0')
            self.instrument.write('SENSE:FOM:RANGE3:COUPLED 0')

            if probe_start is not None:
                self.instrument.write('SENSE:FOM:RANGE1:FREQUENCY:START %f' % probe_start)
            if probe_stop is not None:
                self.instrument.write('SENSE:FOM:RANGE1:FREQUENCY:STOP %f' % probe_stop)

            # self.instrument.write('SENSE:FOM:RANGE2:COUPLED 0')
            # self.instrument.write('SENSE:FOM:RANGE3:COUPLED 0')
            if read_frequency is not None:
                self.instrument.write('SENSE:FOM:RANGE2:FREQUENCY:START %f' % read_frequency)
                self.instrument.write('SENSE:FOM:RANGE2:FREQUENCY:STOP %f' % read_frequency)
                self.instrument.write('SENSE:FOM:RANGE3:FREQUENCY:START %f' % read_frequency)
                self.instrument.write('SENSE:FOM:RANGE3:FREQUENCY:STOP %f' % read_frequency)
            ...
            print("TWO TONE OFF")
            self.instrument.write('SENSE:FOM:RANGE2:COUPLED 1')
            self.instrument.write('SENSE:FOM:RANGE3:COUPLED 1')
            self.instrument.write('SENSE:FOM:RANGE4:COUPLED 0')
            if probe_start is not None:
                self.instrument.write('SENSE:FOM:RANGE1:FREQUENCY:START %f' % probe_start)
            if probe_stop is not None:
                self.instrument.write('SENSE:FOM:RANGE1:FREQUENCY:STOP %f' % probe_stop)
    ...
    def setup_rf_flux_measurement(self, read_power=None, probe_power=None, read_start=None, read_stop=None,
        ...
        if read_start is not None:
            self.instrument.write('SENSE:FOM:RANGE1:FREQUENCY:START %f' % read_start)
        if read_stop is not None:
            self.instrument.write('SENSE:FOM:RANGE1:FREQUENCY:STOP %f' % read_stop)

        if probe_frequency is not None:
            self.instrument.write('SENSE:FOM:RANGE4:FREQUENCY:START %f' % probe_frequency)
            self.instrument.write('SENSE:FOM:RANGE4:FREQUENCY:STOP %f' % probe_frequency)
    ...
    def trigger_single(self, channel=1):
        self.instrument.write('initiate%d:immediate' % channel)
    ...
    def set_trigger_average_mode (self, mode=None):
        ...
        if mode is None:
            ...
            self.instrument.write('sense:AVER:mode ' + mode)
        else:
            print("trigger average mode needs to be one of " + ', '.join(allowed_modes))

    def get_trigger_average_mode (self):
        data = self.instrument.query('sense:AVER:mode?')
        if data is None: return None
        else: return data.strip()

    def set_trigger_average(self, state=True):
        if state:
            self.instrument.write('sense:AVER ON')
        else:
            self.instrument.write('sense:AVER OFF')

    def get_trigger_average(self):
        return bool(self.instrument.query('sense:AVER?'))
    ...
    def set_external_trigger_mode(self, trigger_type, slope=1):
        ...
        if trigger_type.upper() in ["EDGE", "LEVEL"]:
            self.instrument.write("TRIG:TYPE %s"%trigger_type)
            if slope > 0:
                self.instrument.write("TRIG:SLOP POS")
            else:
                self.instrument.write("TRIG:SLOP NEG")
        else:
    ...
    def set_external_trigger_connector(self, connector="MAIN"):
        ...
        if connector.upper() in ["MAIN", "MATH", "PULSE3"]:
            self.instrument.write("TRIG:ROUTE:INP %s" % connector.upper())
    ...
    def get_external_trigger_connector(self):
        """
            ...
            :return: None
            """
        return self.instrument.query("TRIG:ROUTE:INP?")
    ...
    def set_channel_trigger_state(self, state):
        ...
        if state.upper() in ['CHAN', 'POIN', 'SWE', 'TRAC']:
            self.instrument.write("SENS:SWE:TRIG:MODE %s" % state)
    ...
    def get_channel_trigger_state(self):
        ...
        :return:
        """
        return self.instrument.query("SENS:SWE:TRIG:MODE?")

    #### Source
    def set_power(self, power, channel=1, port=1, state=1):
        if state:
            self.instrument.write(":SOURCE%d:POWER%d:MODE ON" % (channel, port))
            self.instrument.write(":SOURCE%d:POWER%d %f" % (channel, port, power))
        else:
            print("Turning off the port %d" %(port))
            self.instrument.write(":SOURCE%d:POWER%d:MODE OFF" % (channel, port))
            # self.instrument.write(":SOURCE%d:POWER%d %f" % (channel, port, power))

    def get_power(self, channel=1, port=1):
        return float(self.instrument.query(":SOURCE%d:POWER%d?" % (channel, port)))
    ...
    def delete_measurement(self, name=None):
        if name is None:
            # delete all traces
            self.instrument.write(r'calc:par:del:all')
        else:
            self.instrument.write("calc:par:del '%s'" % name)
    ...
    def get_measurements(self):
        data = self.instrument.query('calc:par:cat?')
        if data == '"NO CATALOG"\n':
            return None
        else:
    ...
    def set_active_trace(self, channel=1, trace=1, fast=False):
        ...
        if fast:
            query_string += ",fast"
        # print query_string
        self.instrument.write(query_string)
    ...
    def get_active_trace(self, channel=1):
        ...
        data = self.instrument.query(query_string)
        if data is None:
            return data;
        else:
            return int(data)
    ...
    def get_format(self, trace=1):
        ...
        ** Always first assign a MNAME first
        """
    ...
    def read_data(self, sweep_points=None, channel=1, timeout=None, data_format=None):
        ...
        self.instrument.timeout = 16000
        self.get_operation_completion()
        #self.read(timeout=0.1)
        self.instrument.timeout = 16000
        fdat = self.instrument.query("CALC%d:DATA? FDATA" % channel)
        data_str = fdat
