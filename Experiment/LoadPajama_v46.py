...
mingi 12/11/2024
   - added plotfluxmap for S21 measurement as a function of flux (Slabfile, hdf5)
...
from Experiment.fit_codes import selectdomain, fithanger_new_withQc
...
##############################################
# code for high frequency measurement data #
#############################################

import tqdm.notebook as tqdm

def plot_fluxmap(flux_data, dev_ids = [1,2,3,4], uT_per_mAmp = 18.03, plabel = 'field (uT)', scales = [5,5,5,5],CMAP = 'viridis',font = 10, figsize=(16,12)):
    fitmat = []
    devices = {'R1':5400, 'R2':5600, 'R3':5800, 'R4':6000}
    for devkey, data in list(flux_data.items()):
        ...
        #qspecs = [data[1][labels.index(f'R{id}_qspec_mags')] for id in dev_ids]
        mags = [data[1][labels.index(f'R{id}_rspec_mags')] for id in dev_ids]
        
        freqs = [data[1][labels.index(f'R{id}_rspec_freq')][0] for id in dev_ids]
        #qspec_f = data[1][labels.index('qspec_freq')][0]
        flux = data[1][labels.index('flux bias')]
        #field = flux * 18.03  # uT
    
        fields = flux*uT_per_mAmp*1e3
        
        
        
        note = data[1][labels.index('notes')]
        #lenf.append(len(flux))
        #n_skip = lenf[nn] #int(np.around(np.log(len(flux))/np.log(2)))
        #nn += 1
        ...
        for ndev, key in enumerate(list(devices.keys())):
            dellist = []
            ...
            for n in tqdm(range(len(fields))):
        
                mode = devices[key]
                span = 20e6
    
                '''
                select fittinf domiain
                '''
        
                if n < len(mag):
                            # non-nan mask
                    num_nan = np.isnan(mag[n]).sum()
                    non_nan_mask = ~np.isnan(mag[n])
                    freq_meas = freq[non_nan_mask]
                    mag_meas = mag[n][non_nan_mask]
                    mode = freq_meas[np.argmin(mag_meas)]
    
                    if num_nan == 0:
        
                        x_data, y_data = selectdomain(freq_meas, mag_meas, [mode-span/2, mode+span/2])
                        ...
                        y_data = mag_meas
                            ...
                            # non-nan mask
                    num_nan = np.isnan(mag[-1]).sum()
                    non_nan_mask = ~np.isnan(mag[-1])
                    freq_meas = freq[non_nan_mask]
                    mag_meas = mag[-1][non_nan_mask]
                    mode = freq_meas[np.argmin(mag_meas)]
                ...
                fits = fithanger_new_withQc(xdata=x_data, ydata=y_data, fitparams=[f_min, Qin, Qext, 0, scale], domain=[f_min-span/2, f_min+span/2], showfit=False, showstartfit=False,
                                         printresult=False, label=f"{fields[n]}mT", mark_data='b.', mark_fit='r-')
                
                '''
                append fit if it's properly fitted
    
                '''
                
                #fit_array.append(fits)
           
                
                if abs(fits[0] - mode) > 50e6 or fits[1] > 350e3:      # discard fit if frequency moved too much
                    dellist.append(n)
                    # if n == 0:
                    #     mags_norm = np.append(mags_norm, 10**(y_data/10)/fit_array[-1][4])
                    # else:
                    #     mags_norm = np.vstack((mags_norm, 10**(y_data/10)/fit_array[-1][4]))
                
                else:
                    fit_array.append(fits)
                    # if n == 0:
                    #     mags_norm = np.append(mags_norm, 10**(y_data/10)/fits[4])
                    # else:
                    #     mags_norm = np.vstack((mags_norm, 10**(y_data/10)/fits[4]))
            
            #fit_matrices.append(fit_array)
            fitmat00.append(np.array(fit_array))
            fig = plt.figure(figsize=figsize, facecolor='white')
            #i =0
            number_of_scans = np.array(fit_array).shape[0]
            #times = timesweepdata[-2]
            
            fields_cal = np.delete(fields, dellist)
            ...
            #ax1.set_title('time dependent frequency sweep \n {}'.format(gated[i]))
            x = fields_cal #np.delete(fields, dellist)
            y = [fit_array[i][0]/1e6 for i in range(number_of_scans)]
            cm = plt.get_cmap(MAP)
            NPOINTS = len(x)
            ax1.set_prop_cycle(color=[cm(1.*i/(NPOINTS-1)) for i in range(NPOINTS-1)])
            for col in range(NPOINTS-1):
                ax1.plot(x[col:col+2],y[col:col+2],'.-')
            ax1.set_xlabel(plabel)
            ax1.set_ylabel("cavity mode(MHz)")
        
            # y = [fit_array[i][2] for i in range(number_of_scans)]
            # ax2.set_prop_cycle(color=[cm(1.*i/(NPOINTS-1)) for i in range(NPOINTS-1)])
            # for col in range(NPOINTS-1):
            #     ax2.plot(x[col:col+2],y[col:col+2],'.-')
            # ax2.set_ylabel("Qex")
            # ax2.set_xlabel(plabel)
            # ax2.set_ylim(2e4, 8e4)
            
            y = [fit_array[i][1] for i in range(number_of_scans)]
            ax4.set_prop_cycle(color=[cm(1.*i/(NPOINTS-1)) for i in range(NPOINTS-1)])
            for col in range(NPOINTS-1):
                ax4.plot(x[col:col+2],y[col:col+2],'.-')
            ax4.set_ylabel("Qin")
            ax4.set_xlabel(plabel)
            
        
    
        
        
            # y = [fit_array[i][3]/1e3 for i in range(number_of_scans)]
            # ax5.set_prop_cycle(color=[cm(1.*i/(NPOINTS-1)) for i in range(NPOINTS-1)])
            # for col in range(NPOINTS-1):
            #     ax5.plot(x[col:col+2],y[col:col+2],'.-')
            # ax5.set_ylabel("delta f (kHz)")
            # ax5.set_xlabel(plabel)
        
            cmap ='RdBu' #cmap ='Greys', cmap = 'gnuplot'
            #ax5 = fig.add_subplot(2,4,(2,4))
            ax5.set_title(f'Field response \n {note[0]} \n {devkey}', wrap=True)
            spec1 = ax5.pcolor(np.arange(0, mag.shape[0],1), freq/1e6,  np.transpose(mag),shading="auto",cmap=cmap) #cmap='RdBu'
            ax5.set_xlabel(plabel)
            ax5.set_ylabel('Frequency (MHz)')
            #ax5.axvline(x=xdata[xcol], color='yellow', linewidth=2)
            ax5.set_xticks(np.arange(0, len(x), 10))
            ax5.set_xticklabels(np.around(x[0::10], 3))
            ax5.axvline(np.argmax(x), color='yellow')
            ax5.axvline(np.argmin(x), color='yellow')
            # if 'zlim1' in kwargs:
            #     zlim1 = kwargs['zlim1']
            #     spec1.set_clim(zlim1[0], zlim1[1])
            cbar = fig.colorbar(spec1, ax=ax5)
            #cbar = fig.colorbar(spec, ax=ax1)
            cbar.set_label(r' $S_{{21}}$')
            #spec1.set_clim(0.4, 1.1)
            #print(f'{len(dellist)} del \n {fluxsweeps[i]}')
            
            
            #fig.suptitle('field sweep \n {}'.format(demags[i]))
            
            plt.tight_layout()
            plt.show()

        fitmat.append(fitmat00)
